...
class HeroSkills {
	...
	public static void addDailySkills() {
		selectHeroPosition();
		for (;;) {
			if (getLeftHero().contains(currentHero)) {
				//select left hero
				GameMouse.click(180, 200);
				addSkillsToHero();
			} else if (getRightHero().contains(currentHero)) {
				//select right hero
	...
	public static void selectHeroPosition() {
		// click Position (All, Front, Central, Back)
		GameMouse.click(120 * position + 20, 120);
	}

	public static void addSkillsToHero() {
		// click Skills
		GameMouse.click(175, 450);
		// click all four skills
		GameMouse.click(280, 185);
		GameMouse.click(280, 262);
		GameMouse.click(280, 340);
		GameMouse.click(280, 418);

		GameMouse.hitEscape();
	}

	public static String getLeftHero() {
		Tesseract.takeScreenshot(164, 175, 200, 30);
		return Tesseract.parseScreenshotString();
	}

	public static String getRightHero() {
		Tesseract.takeScreenshot(497, 175, 200, 30);
		return Tesseract.parseScreenshotString();
	}

	public static void goToNextHeroes() {
		GameMouse.mouseWheel(700, 162, true, new GameMouseWheelListener() {
    		public boolean shouldKeepScrolling() {
        		Color color = GameMouse.getPixelColor(700, 162);
